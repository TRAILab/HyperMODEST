...
def adaptive_filter(ptc_rect, pp_score, obj, low_score, high_score, percentile=50, threshold=0.5):
    if not filter_by_ppscore(ptc_rect, pp_score, obj, percentile, threshold):
        if obj.score > high_score:
            return True
        else: 
            return False
    if obj.score < low_score:
        return False
    return True
...
def main(args: DictConfig):
    ...
    # Compute the score threshold for pseudo-labels filtering.
    all_scores=np.array([])
    for idx, det_bbox in zip(tqdm(idx_list), det_bboxes):
        det = predicts2objs(det_bbox)
        for obj in det:
            all_scores=np.append(all_scores, obj.score)
    
    score_threshold = np.percentile(all_scores, 25, method="nearest")
    print("Score threshold at 25% percentile is ", score_threshold)

    score_threshold_keep_static = 0.8
    print("Score threshold used to keep static labels is ", score_threshold_keep_static)
    ...
    for idx, det_bbox in zip(tqdm(idx_list), det_bboxes):
        ...
        det_obj = list(filter(
            lambda obj: adaptive_filter(
                ptc_in_rect, pp_score, obj, score_threshold,
                score_threshold_keep_static,
