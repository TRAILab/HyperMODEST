...
class KittiDataset(DatasetTemplate):
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train', score_filter=False):
        import torch
        ...
        if score_filter:
            # Compute the score threshold, 25% percentile of all confidence scores. 
            all_scores = np.array([])
            for k in range(len(infos)):
                info = infos[k]
                sample_idx = info['point_cloud']['lidar_idx']
                ...
                for i in range(num_obj):
                    ...
                    if (used_classes is None) or names[i] in used_classes :
                        ...
                        if names[i] in all_db_infos:
                            ...
                            all_db_infos[names[i]] = [db_info]
                        all_scores = np.append(all_scores,annos['score'][i])

            score_threshold = np.percentile(all_scores, 25, method="nearest")
            print("score threshold", score_threshold)

            # Write the result to the pseudo ground-truth database for data agumentation. 
            all_db_infos = {}
            for k in range(len(infos)):
                print('gt_database sample: %d/%d' % (k + 1, len(infos)))
                info = infos[k]
                ...
                for i in range(num_obj):
                    ...
                    if ((used_classes is None) or names[i] in used_classes) and annos['score'][i] > score_threshold:
                        db_path = str(filepath.relative_to(self.root_path))  # gt_database/xxxxx.bin
                        db_info = {'name': names[i], 'path': db_path, 'image_idx': sample_idx, 'gt_idx': i,
                                'box3d_lidar': gt_boxes[i], 'num_points_in_gt': gt_points.shape[0],
                                'difficulty': difficulty[i], 'bbox': bbox[i], 'score': annos['score'][i]}
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, if_val=True, workers=4):
    ...
    dataset.set_split(train_split)
    score_filter = True
    dataset.create_groundtruth_database(train_filename, split=train_split, score_filter=score_filter)
